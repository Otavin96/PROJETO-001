...
import { SupplierId, SuppliersRepository } from "@/suppliers/repositories/suppliers.repository";

export class SuppliersInMemoryRepository extends InMemoryRepository<SuppliersModel> implements SuppliersRepository {
  ...
  protected async applyFilter(items: SuppliersModel[], filter: string | null): Promise<SuppliersModel[]> {
    if(!filter) return items
      return items.filter(item => 
      item.name.toLowerCase().includes(filter.toLowerCase()))
  }

  protected async applySort(items: SuppliersModel[], sort: string | null, sort_dir: string | null): Promise<SuppliersModel[]> {
